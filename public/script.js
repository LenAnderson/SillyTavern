...
async function firstLoadInit() {
    ...
    await updateLoaderStatus('initializing secrets', await initSecrets());
    await updateLoaderStatus('checking API keys', readSecretState());
    await updateLoaderStatus('loading UI translations', initLocales());
    updateLoaderStatus('initializing chat utilities', initChatUtilities());
    updateLoaderStatus('registering additional slash commands', initDefaultSlashCommands());
    ...
    await updateLoaderStatus('loading user avatars', getUserAvatars(true, user_avatar));
    await updateLoaderStatus('loading characters', getCharacters());
    await updateLoaderStatus('loading backgrounds', getBackgrounds());
    await updateLoaderStatus('initializing tokenizers', initTokenizers());
    initBackgrounds();
    initAuthorsNote();
    await updateLoaderStatus('loading personas',
        initPersonas(),
    );
    updateLoaderStatus('initializing World Info',
        initWorldInfo(),
    );
